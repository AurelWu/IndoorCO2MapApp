...
namespace IndoorCO2MapAppV2.Util
{
    internal partial class NaturalStringComparer
    {
        public static int Compare(string x, string y)
        {
            ...
            var tokensX = NaturalStringComparer.Tokenize(x);
            var tokensY = NaturalStringComparer.Tokenize(y);
        ...
        private static List<string> Tokenize(string input)
        {
            ...
            foreach (Match match in TextNumberSplitterRegex().Matches(input))
            {
        ...
        [GeneratedRegex(@"\D+|\d+")]
        private static partial Regex TextNumberSplitterRegex();
    }
}
