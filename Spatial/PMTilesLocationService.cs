...
{
    internal sealed class PMTilesLocationService
    {
        private static readonly Lazy<PMTilesLocationService> _instance = new(() => new PMTilesLocationService());
        public static PMTilesLocationService Instance => _instance.Value;

        private const string TileUrl = "https://indoorco2map-planet.s3.eu-central-1.amazonaws.com/tiles/co2map.pmtiles";
        private const int Zoom = 14;

        private static readonly HttpClient _http = new();

        // Cached header + root directory (valid 24 h)
        private PmHeader? _header;
        private List<DirEntry>? _rootDir;
        private DateTime _cacheTime = DateTime.MinValue;
        private readonly SemaphoreSlim _initLock = new(1, 1);

        private PMTilesLocationService() { }
        ...
        public async Task<List<LocationData>> SearchAsync(
        ...
        private async Task<(PmHeader header, List<DirEntry> rootDir)> EnsureHeaderAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (Encoding.ASCII.GetString(hBytes, 0, 7) != "PMTiles")
                    throw new InvalidDataException("Not a PMTiles file");
                ...
                Logger.WriteToLog($"PMTiles: header loaded, {rootDir.Count} root entries");
                return (h, rootDir);
            }
            finally
            {
        ...
        // ── Directory parsing ─────────────────────────────────────────────────
        //
        // PMTiles v3 columnar varint layout:
        //   numEntries       varint
        //   tileId deltas[]  varint  (cumulative sum → actual tile IDs)
        //   runLengths[]     varint  (0 = leaf dir pointer, >0 = tile entry)
        //   lengths[]        varint
        //   offsets[]        varint  (0 = prev.offset + prev.length; else stored-value - 1)
        ...
        private static void DecodeLayerInto(
            byte[] data, double userLat, double userLon,
            long tx, long ty, int rangeMeters,
            HashSet<string> seen, List<LocationData> results)
        {
            // Tile message: field 3 = layers
            int pos = 0;
            while (pos < data.Length)
            {
                ...
                if (f == 3 && w == 2)
                {
                    ...
                    DecodeLayerMessage(data.AsSpan(pos, len).ToArray(), userLat, userLon, tx, ty, rangeMeters, seen, results);
                    pos += len;
                }
                else
                {
        ...
        // Layer message: field 1=name, 2=features, 3=keys, 4=values, 5=extent
            ...
            HashSet<string> seen, List<LocationData> results)
        {
            ...
            while (pos < data.Length)
            {
                ...
                if      (f == 1 && w == 2)  ReadString(data, ref pos); // layer name - skip
            ...
            foreach (var fd in rawFeatures)
            {
                ...
                string osmType  = props.TryGetValue("@type", out var ot) ? ot : "way";
                string osmIdStr = props.TryGetValue("@id",   out var oi) ? oi : "";

                // Dedup across tiles
                string dedupeKey = $"{osmType}:{osmIdStr}";
                if (!string.IsNullOrEmpty(osmIdStr) && !seen.Add(dedupeKey)) continue;

                string name  = props.TryGetValue("name",  out var nm) ? nm : "";
                string brand = props.TryGetValue("brand", out var br) ? br : "";
                if (string.IsNullOrEmpty(name) && string.IsNullOrEmpty(brand)) continue;

                string displayName = name;
                if (!string.IsNullOrEmpty(brand) && brand != name)
                    displayName = string.IsNullOrEmpty(name) ? brand : $"{name} ({brand})";
                if (string.IsNullOrEmpty(displayName)) continue;

                long osmId = long.TryParse(osmIdStr, out var parsed) ? parsed : 0L;
                var loc = new LocationData(osmType, osmId, displayName, flat, flon, userLat, userLon);
                if (loc.Distance <= rangeMeters)
                    results.Add(loc);
            }
        }

        // Feature message: field 1=id, 2=tags, 3=type, 4=geometry
            ...
            byte[] data, List<string> keys, List<string> values)
        {
            ...
            while (pos < data.Length)
            {
                ...
                if (f == 2 && w == 2) // tags (key/value index pairs)
                {
                    int len = (int)ReadVarint(data.AsSpan(), ref pos);
                    int end = pos + len;
                ...
                else if (f == 4 && w == 2) // geometry — only read first MoveTo
                {
                    int len = (int)ReadVarint(data.AsSpan(), ref pos);
                    int end = pos + len;
                    if (end > data.Length) break;
        ...
        // Value message: field 1=string, 2=float, 3=double, 4=int64, 5=uint64, 6=sint64, 7=bool
        private static string DecodeValue(byte[] data)
        {
            ...
            while (pos < data.Length)
            {
                ...
                if (f == 6 && w == 0)
                {
                    ulong v = ReadVarint(data.AsSpan(), ref pos);
                    return ((long)(v >> 1) ^ -(long)(v & 1)).ToString();
                }
        ...
        {
            // Detect format by magic bytes (overrides header flag)
        ...
        private static void SkipField(byte[] data, ref int pos, int wireType)
        {
            switch (wireType)
            {
                ...
                case 2:
                    int skip = (int)ReadVarint(data.AsSpan(), ref pos);
                    pos = Math.Min(pos + skip, data.Length);
                    break;
